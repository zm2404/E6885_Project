...
class WaterSortEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, num_bottles:int,  water_level:int, num_empty_bottles:int=2):
        ...
        self._observation_spec = {
            ...
            'action_mask': array_spec.ArraySpec(
                shape=(self.num_bottles*(self.num_bottles-1), ), dtype=bool, name='action_mask') # dtype=? 待定
        }
    ...
    def action_mask(self) -> list[int]:
        ...
        for i in range(len(action_mask)):
            ...
            if self.bottles_capacity[origin_bottle] != 0:           # if the origin bottle is not empty. (if empty, no action can be taken)
                if self.bottles_capacity[dest_bottle] == 0:         # if the destination bottle is empty, any color can be poured in
                    ...
                    if self._state[origin_bottle][-origin_bottle_capacity] == self._state[dest_bottle][-dest_bottle_capacity]:  # 假设右边是底
                        action_mask[i] = 1
    ...
    def new_game(self,new:bool) -> tuple[np.ndarray, list[int], int, int]:
        ...
        if not new:
            ...
            bottles_color,bottles_capacity,maxstep = self.generate_game(30)
        
        print(maxstep)
        return bottles_color, bottles_capacity, 0, 0
    

    def generate_game(self,maxsteps:int) -> np.ndarray:
        '''
    ...
    def pour_water(self, origin_bottle:int, dest_bottle:int):
        ...
        '''
        self.num_moves += 1
        top_color = self._state[origin_bottle][-self.bottles_capacity[origin_bottle]]  # get the top color of the origin bottle
        while self.bottles_capacity[origin_bottle] > 0 and self.bottles_capacity[dest_bottle] < self.water_level:
            if self._state[origin_bottle][-self.bottles_capacity[origin_bottle]] == top_color:    # if the color is the same as the top color
                self._state[origin_bottle][-self.bottles_capacity[origin_bottle]] = 0             # continue to pour
                self.bottles_capacity[origin_bottle] -= 1
                self.bottles_capacity[dest_bottle] += 1
                self._state[dest_bottle][-self.bottles_capacity[dest_bottle]] = top_color
            else:
                break
    ...
    def is_game_over(self) -> str:
        ...
        if not any(action_mask):        # if all action_mask are False, then no actions can be taken and the game is over
            ...
            if self.bottles_capacity.count(0) == self.num_empty_bottles:    # if all bottles are full except two empty bottles
